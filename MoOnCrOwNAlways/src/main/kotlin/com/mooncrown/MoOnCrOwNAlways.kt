...
private suspend fun MoOnCrOwNAlways.getOrFetchPlaylist(): Playlist {
...
private suspend fun MoOnCrOwNAlways.createSearchResponse(cleanTitle: String, shows: List<PlaylistItem>): SearchResponse? {
...
private suspend fun MoOnCrOwNAlways.fetchTMDBData(title: String): Pair<JSONObject?, TvType> {
    return withContext(Dispatchers.IO) {
        try {
            ...
            if (tvResults != null && tvResults.length() > 0) {
                ...
                if (tvId != null) {
                    val detailsUrl = "https://api.themoviedb.org/3/tv/$tvId?api_key=$apiKey&append_to_response=credits,seasons&language=tr-TR"
...
// Yeni yardımcı fonksiyon: Tek bir bölümün (Episode) TMDB detaylarını çeker.
private suspend fun MoOnCrOwNAlways.fetchTMDBEpisodeData(
    tmdbId: Int,
    seasonNumber: Int,
    episodeNumber: Int
): JSONObject? {
    return withContext(Dispatchers.IO) {
        try {
            val apiKey = "4032c1fd53e1b6fef5af1b406fccaa72"
            val episodeUrl =
                "https://api.themoviedb.org/3/tv/$tmdbId/season/$seasonNumber/episode/$episodeNumber?api_key=$apiKey&language=tr-TR"
            val response = URL(episodeUrl).readText()
            JSONObject(response)
        } catch (e: Exception) {
            Log.e("TMDB", "TMDB bölüm verisi çekilirken hata oluştu: S$seasonNumber E$episodeNumber", e)
            null
        }
    }
}
...
override suspend fun load(url: String): LoadResponse {
    ...
    // TMDB'den sezon özetlerini (plot) çekmek için bir harita oluşturun
    val seasonPlots = (0 until (tmdbSeasonsArray?.length() ?: 0)).mapNotNull { i ->
        tmdbSeasonsArray?.optJSONObject(i)?.let { seasonJson ->
            seasonJson.optInt("season_number") to (seasonJson.optString("overview") ?: "")
        }
    }.toMap()
    ...
    groupedEpisodes.forEach { (key, episodeItems) ->
        ...
        // Eğer bölüm için TMDB özeti yoksa, sezon özetini dene
        if (episodePlot.isNullOrBlank()) {
            episodePlot = seasonPlots[finalSeason]?.takeIf { it.isNotBlank() }
        }
        
        // Bölüm adını TMDB'den gelen veri ile zenginleştirme
        val episodeName = tmdbEpisodeData?.optString("name")?.takeIf { it.isNotBlank() }
        val finalName = if (episodeName != null) {
            "${itemCleanTitle} S$finalSeason E$finalEpisode - $episodeName"
        ...
        val episodeLoadData = LoadData(
            ...
            isDubbed = isDubbed,
            isSubbed = isSubbed,
            score = item.score // IPTV'deki puanı kullan
        )

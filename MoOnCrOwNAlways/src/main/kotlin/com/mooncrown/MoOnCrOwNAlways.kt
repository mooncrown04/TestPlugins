...
        // matchResult5 sadece 2 değer döndürüyor, bu nedenle sezonu 1 olarak varsayalım.
        // Format5: (.*?) S(\d+)E(\d+).* -> (title, seasonStr, episodeStr)
        return Triple(title.trim(), seasonStr.toIntOrNull(), episodeStr.toIntOrNull())
    }
...
        val episodePoster = item.attributes["tvg-logo"]?.takeIf { it.isNotBlank() } ?: (tmdbEpisodePosterUrl ?: finalPosterUrl)
        
        // Bölüm LoadData'sına TMDB detaylarını eklemek için yeni bir alan eklenebilir, 
        // ancak mevcut LoadData yapısını bozmamak için sadece posteri Episode objesine ekleyelim.
        // Plot'u Episode objesine ekleyemediğimiz için şimdilik dışarıda tutuyoruz.
...
            this.name = if (finalEpisode > 0) {
                "${itemCleanTitle} S$finalSeason E$finalEpisode - $episodeName" // TMDB bölüm adını ekledik
            } else {
                itemCleanTitle
            }

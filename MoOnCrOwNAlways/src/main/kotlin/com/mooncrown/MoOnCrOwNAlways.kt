...
class MoOnCrOwNAlways: MainAPI() {
    ...
    // EPG URL'sini burada tanımlıyoruz
    override val tvgUrl = "https://raw.githubusercontent.com/ahmethascelik/epghost/refs/heads/main/xmltv.xml"
    override val hasEpg = true
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        // SES BİLGİSİNİ BURADA ALIYORUZ!
        val audioInfo = coroutineScope {
            try {
                // Yayının URL'sine bir istek gönderiyoruz
                val streamContent = app.get(firstUrl).text

                // HLS yayınlarında ses seçenekleri genellikle #EXT-X-MEDIA etiketi ile gelir.
                // Basit bir kontrol ile bu etiketteki "LANGUAGE" veya "NAME" bilgilerini arayabiliriz.
                val audioTracks = mutableListOf<String>()
                streamContent.lineSequence().forEach { line ->
                    if (line.contains("EXT-X-MEDIA:TYPE=AUDIO")) {
                        val language = Regex("LANGUAGE=\"(.*?)\"").find(line)?.groupValues?.getOrNull(1)
                        if (language != null) {
                            audioTracks.add(language.uppercase())
                        }
                    }
                }

                if (audioTracks.isNotEmpty()) {
                    "Ses Dilleri: " + audioTracks.distinct().joinToString(", ")
                } else {
                    "Yayın akışında ses bilgisi bulunamadı."
                }
            } catch (e: Exception) {
                Log.e("MoOnCrOwNTV", "Yayın bilgileri alınırken hata oluştu: ${e.message}")
                "Ses bilgisi alınamadı."
            }
        }

        return newLiveStreamLoadResponse(loadData.title, firstUrl, url) {
            this.posterUrl = loadData.poster
            this.plot = nation + "\n\n" + audioInfo // Açıklama kısmına ses bilgisini ekliyoruz.
            this.tags = listOf(loadData.group, loadData.nation)
            this.recommendations = uniqueRecommendations
            // Cloudstream'in EPG verisini otomatik olarak alması için eklendi
            this.tvgUrl = this@MoOnCrOwNAlways.tvgUrl
            this.tvgId = loadData.title
        }
    }

...
suspend fun invokeFilmModu(
    ...
    if (filmModuAPI.isEmpty() || title.isNullOrBlank()) return

    fun String.ensureAbs(base: String): String {
        return if (startsWith("http")) this
        else base.trimEnd('/') + "/" + trimStart('/')
    }

    // 🔎 Arama
    val searchUrl = "$filmModuAPI/film-ara?term=${title.trim()}"
    val searchDoc = app.get(searchUrl, referer = "$filmModuAPI/").document

    val matchATag = searchDoc.select("div.movie a")
        .firstOrNull { it.text().equals(title.trim(), ignoreCase = true) }
        ?: searchDoc.selectFirst("div.movie a") ?: return

    val detailUrl = matchATag.attr("href").ensureAbs(filmModuAPI)
    val detailDoc = app.get(detailUrl, referer = "$filmModuAPI/").document

    // 🎯 Yıl kontrolü (varsa)
    detailDoc.selectFirst("span[itemprop='dateCreated']")?.text()?.trim()?.toIntOrNull()?.let { foundYear ->
        if (year != null && foundYear != year) return
    }

    // 📺 Sezon/Bölüm (varsa) – bulunamazsa film sayfasıyla devam eder
    val targetUrl = if (season != null && episode != null) {
        detailDoc.select("a:matches((?i)Sezon\\s*$season)")
        detailDoc.select("a:matches((?i)Bölüm\\s*$episode)")
            .firstOrNull()?.attr("href")?.ensureAbs(filmModuAPI) ?: detailUrl
    } else detailUrl

    val pageDoc = app.get(targetUrl, referer = detailUrl).document

    // 🎥 Alternatif oynatıcılar
    pageDoc.select("div.alternates a").forEach { alt ->
        val altName = alt.text()
        if (altName.contains("Fragman", ignoreCase = true)) return@forEach

        val altHref = alt.attr("href").takeIf { it.isNotBlank() }?.ensureAbs(filmModuAPI) ?: return@forEach
        val altPage = app.get(altHref, referer = detailUrl).text

        val vidId   = Regex("""var\s+videoId\s*=\s*'([^']+)'""").find(altPage)?.groupValues?.get(1) ?: return@forEach
        val vidType = Regex("""var\s+videoType\s*=\s*'([^']+)'""").find(altPage)?.groupValues?.get(1) ?: return@forEach

        val apiUrl = "$filmModuAPI/get-source?movie_id=$vidId&type=$vidType"
        val src = app.get(apiUrl, referer = "$filmModuAPI/").parsedSafe<GetSource>() ?: return@forEach

        // 📝 Altyazı (varsa)
        src.subtitle?.let { subUrl ->
            subtitleCallback.invoke(SubtitleFile("Türkçe", subUrl.ensureAbs(filmModuAPI)))
        }

        // 🔗 Kaynaklar
        src.sources?.forEach { s ->
            val streamUrl = s.src.ensureAbs(filmModuAPI)
            callback.invoke(
                newExtractorLink(
                    source = "FilmModu",
                    ...
                    this.referer = "$filmModuAPI/"
                    this.headers = mapOf("Referer" to "$filmModuAPI/")
                    this.quality = getQualityFromName(s.label)
                }
            )
        }
    }
